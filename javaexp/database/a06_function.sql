...
2. sql 함수의 유형
	...
	2) 복수행 함수 : 조건에 따라 여러 행을 그룹화하여 그룹별로 결과를 하나씩 반환하는 함수
		여러 행의 데이터를 함수가 적용되었을때 단일 또는 줄어든 행의 결과값을 리턴하는 함수를 말한다
...
2. 문자 함수의 종류
	...
	2) 문자조작 함수
		...
		substr(문자열데이터, 시작위치, 시작위치에서 갯수)
			문자열데이터를 시작위치와 마지막위치를 기준으로 절삭처리하여 사용하는 것을 말한다
			index : 0부터 시작하는 경우/ 1부터 시작하는 경우
				oracle에서 substr은 1부터 시작한다! ***
			...
			ex) 사원명 : [AB ] [검색]
				...
				2) 대소문자 구분없이 검색되게 처리
					DB테이블
						HELLO
					검색 Hel시 해당내용을 검색하지않으면 검색이안된다
						HEllo, hellO
						hello, HELLO로도 검색이 되게하려면
...
-- ex) 함수를 이용하여 다음과 같은 형태로 출력하세요. 활용컬럼(ename, job), 조건문 소문자k 포함할때
...
FROM EMP e 
WHERE LOWER(ENAME) LIKE '%k%'; 
--WHERE LOWER(ename)  LIKE '%'||upper('k')||'%';
...
SELECT CONCAT(empno, ename) show FROM EMP e ;
-- substr(문자열데이터, 시작위치, 시작위치에서 갯수)
SELECT substr('sql*plus',5,4) substr01 FROM dual;
--	1부터 시작하는 5번째 p문자부터 4개를 추출해서 출력한다
--	ex) 951212-2701521 본인의 주민번호를 기준으로 substr을 화룡ㅇ하여 @@년 @@월 @@일 생일하고 표현하세요
SELECT substr('951212-2701521',0,2)||'년 '||substr('951212-2701521',3,2)||'월 '||substr('951212-2701521',5,2)||'일 생일' show
FROM dual;
-- 문자열 길이 length, lengthb
SELECT length('hello') str1, lengthb('hello') str2, length('안녕하세요') str3, lengthb('안녕하세요') str4 FROM dual;
SELECT ename, length(ename)
FROM emp;
-- ex) 사원명과 직책의 글자수를 사원명, 직책명과 함께 출력하되 직책의 글자수가 6이상인 경우만 검색
