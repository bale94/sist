...
# merge
1. 개념
	1) 구조가 같은 두 개의 테이블을 비교하여 하나의 테이블로 합치기위한 데이터 조작어
	2) when절의 조건절에서 결과 테이블에 해당 행이 존재하면 update 명령문에 의해 새로운 값으로 수정
		그렇지않으면 insert 명령문으로 새로운 행을 삽입
	3) 대량의 데이터를 분석하기 위한 업무에 유용
	4) 전자상거래에서 상품정보를 등록하는데 해당 상품이 등록되어 있으면 update로 상품정보가 수정이되고,
		상품이 등록되어 있지않으면 상품정보를 수정처리되게 하는 경우에 사용된다
2. 기본형식
	merge into 목적테이블명
...
AS SELECT * FROM emp;
SELECT * FROM emp03;
-- 사원번호 기준으로 해당 데이터가 있으면 수정처리, 데이터가 없으면 등록처리
--# merge를 쓰지 않을때
...
-- ex) emp06(등록/수정할 테이블), emp05(입력할 테이블)
CREATE TABLE emp05
