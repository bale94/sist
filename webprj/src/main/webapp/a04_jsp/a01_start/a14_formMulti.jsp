...
<body>
	<form>
		...
		<%--
			select는 value를 속성으로 지정하지 않을때는 레이블에 있는 내용이 value로 처리되어
			전달되지만 따로 option을 활용하여 value를 지정하면 지정된 값이 전달되어 온다
		...
		<%--
		여러가지중에 선택되어진 것의 value값을 전송한다.
		radio는 같은 이름(name)의 범위 안에서만 단일선택 되어진다.
		--%><br>
		추천인:
		<input type="text" name="name01" value="홍길동" size="3"/>
		<input type="text" name="name01" value="김길동" size="3"/>
		<input type="text" name="name01" value="신길동" size="3"/><br>
		추천인2:
	...
	넘겨온값:<%=request.getParameter("cho01") %><br>
	넘겨온값:<%=request.getParameter("cho02") %><br>
	넘겨온값:<%=request.getParameter("cho03") %><br>
	넘겨온값:<%=request.getParameter("cho04") %><br>
	넘겨온값:
	<%
