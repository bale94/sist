...
# 페이지 모듈화
1. <jsp:include> 액션태그
	1) 다른 jsp페이지의 실행결과를 현재위치에 삽입
2. 구문
	1) <jsp:include page="포함할페이지" flush="true"/>
	2) page 속성: 포함할 jsp페이지
	3) flush 속성: 지정한 jsp페이지를 실행하기 전에 출력 버퍼를 플러시 할 지의 여부를 지정한다
					true이면 출력버퍼를 플러시하고 false이면 하지않는다.
3. <jsp:param> 액션태그
	1) 신규 파라미터를 추가하는데 사용
