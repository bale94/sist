...
if(ename==null) ename=""; // 초기화면에 전체검색처리
if(job==null) job=""; // 초기화면에 전체검색처리
%>
...
<%-- 부서정보 등록페이지 만들고, 이벤트에 의해서 버튼으로 등록페이지 이동 --%>
<%
A04_PreDAO dao = new A04_PreDAO();
/*
# 요청값에 의한 검색 처리
1. form에 있는 요청값을 받기
2. 조건문에 의해서 null값에 대한 처리
3. DAO에 vo객체 넘겨주기
*/
...
<table id="customers">
	...
	<tr ondblclick="goDetail(<%=e.getEmpno()%>)"><td><%=e.getEmpno()%></td><td><%=e.getEname()%></td><td><%=e.getJob()%></td><td><%=e.getSal()%></td><td><%=e.getDeptno()%></td></tr>
	<%}%>
</table>
</body>
</html>