...
public class A01_Basic {
	...
	public static void main(String[] args) {
		...
		4. 예외처리 기본 코드
			1) 예외 발생시 프로그램 종료 막고, 정상 실행 유지할 수 있도록 처리
				- 일반 예외 : 반드시 작성해야 컴파일 가능
					IO가 일어나는 경우, 메모리 사용하는 경우
					DB연동처리
...
//		System.out.println(1/0);	// 에러가 발생하면 더 이상 프로세스가 진행하지 않음.
		...
		try {
			...
			// 예외가 발생하는 시점에서 바로 catch블럭으로 예외를 던진다.
		...
		//		위 코드를 try{}catch 구문없이 처리하고, 다시 try{} catch 구문으로
				// 처리하여 예외 프로세스가 어떻게 흘러가는지를 확인하세요.
		String name = null;
...
//			System.out.println(name.toString());
		...
		/*
		# 예외 처리 프로그램 진행하기(runtime예외)
		1. 예외가 나올 수 있는 코드를 작성한다.
		2. 실행 후, console창에 발생한 예외를 복사한다.
		...
		// ex) System.out.println(Integer.parseInt("이십오"));
			// 위 코드로 발생하는 예외를 처리하고 처리한결과를 수행하세요
